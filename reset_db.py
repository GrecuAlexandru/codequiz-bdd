...
def main():
    ...
    try:
        ...
        # 1. Schema (Drops and Recreates Tables)
        execute_sql_file(conn, 'schema.sql')
        
        # 2. Procedures (Drops and Recreates Stored Procs)
        execute_sql_file(conn, 'procedures.sql')
        
        # 3. Seed Data (OPTIONAL - Commented out for empty DB request)
        execute_sql_file(conn, 'seed_data.sql')
        
        # Cleanup artifacts
        try:
             conn.execute_query("IF OBJECT_ID('TestTable', 'U') IS NOT NULL DROP TABLE TestTable")
